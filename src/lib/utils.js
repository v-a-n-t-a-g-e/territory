  ...
  try {
    ...
    if (buildingGeometry) {
      ...
      if (textureBuildings && mapCanvas) {
        ...
        if (tops)  group.add(new THREE.Mesh(tops,  new THREE.MeshStandardMaterial({ map: texture })));
    ...
    if (groundObj) {
      groundObj.traverse((child) => {
        ...
        if (textureGround && child.material?.map) {
          ...
          child.material = new THREE.MeshStandardMaterial({ map: tex, side: THREE.DoubleSide });
        } else {
          child.material.dispose();
